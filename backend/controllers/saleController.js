...
    console.error('Error creating sale:', error.stack); // Log full error stack
...
    if (search) {
      if (!isNaN(search) && parseFloat(search).toString() === search) {
        where[Op.or] = [
          { id: parseInt(search) },
          { totalAmount: parseFloat(search) }
        ];
      } else {
        // CORRECTED: Changed Op.iLike to Op.like for SQLite compatibility
...
        model: SaleItem,
        as: 'items',
        attributes: ['quantity', 'priceAtSale', 'productId'], // Make sure productId is included
        include: [
          {
            model: Product,
            as: 'product',
...
    }
    ...
    // --- Start CORRECTED count logic for SQLite ---
    let countWhere = { ...where }; // Start with conditions directly on Sale model
    let countIncludeOptions = [];

    // Add customer include for count if customer search is active
    if (Object.keys(customerWhere).length > 0) {
        countIncludeOptions.push({
            model: Customer,
            as: 'customer',
            attributes: [],
            where: customerWhere,
            required: true // Must be true for filtering in count
        });
    }

    // Add SaleItem and Product include for count if product search is active
    if (Object.keys(productWhere).length > 0) {
        countIncludeOptions.push({
            model: SaleItem,
            as: 'items',
            ...
            required: true // SaleItem must be required if its product is filtered
        });
    }

    // Perform count query separately for robustness with SQLite and complex joins
    const count = await Sale.count({
        where: countWhere,
        include: countIncludeOptions,
        distinct: true, // Count distinct sales (important for sales listing)
        col: 'Sale.id' // Specify the column to count distinctly, referring to the top-level Sale model
    });
    // --- End CORRECTED count logic for SQLite ---

    const rows = await Sale.findAll({
      where, // Use original 'where' for data retrieval
      order: [['saleDate', 'DESC']],
      limit: parseInt(limit),
      offset: offset,
      include: includeOptions // Use the already configured include options for selecting data
    });
...
    });
  } catch (err) {
    console.error('Failed to fetch sales:', err.stack); // Log full error stack
...
    }
  } catch (err) {
    console.error('Server error fetching sale by ID:', err.stack); // Log full error stack
...
    console.error('Sale update failed:', err.stack); // Log full error stack
...
  } catch (err) {
    if (transaction) await transaction.rollback();
    console.error('Sale deletion failed:', err.stack); // Log full error stack
...
    }
  } catch (err) {
    console.error('Server error fetching invoice data:', err.stack);
    res.status(500).json({ error: 'Server error', details: err.message });
  }
};
